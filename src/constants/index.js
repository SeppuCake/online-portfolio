...
const projects = [
  ...
  },
  {
    ...
    longDescription:
      "This project was developed as a freelance commission for a degree student's final year project at a local Malaysian university, serving as a proof-of-concept implementation of anisotropic diffusion — a non-linear image processing technique that selectively smooths noise within regions while preserving structural edges. The application was built entirely in Java and allows users to load any image and apply three distinct diffusion models: the classic Perona-Malik formulation (Perona & Malik, 1990), the robust estimation approach by Black et al. (1998), and a conductance-based method. Each model computes pixel-level diffusion iteratively using partial differential equations (PDEs), producing progressively denoised output while keeping edge boundaries sharp. The core challenge of this project was translating dense academic mathematics directly into executable Java code — for example, the Perona-Malik conductance function g₁(x) = exp[−(x/K)²] was implemented as a per-pixel exponential calculation across the image matrix, where x represents the local gradient magnitude and K is the edge sensitivity constant. Similarly, the discrete diffusion update equation ∂I/∂t = div(c(‖∇I‖)∇I) was approximated iteratively by computing finite differences between neighbouring pixel intensities in four cardinal directions (North, South, East, West), multiplying each directional gradient by the conductance value, and summing the weighted contributions to update each pixel. The Black et al. model extended this by replacing the conductance function with Tukey's biweight robust estimator, requiring implementation of a non-linear influence function that rejects gradient outliers — effectively preserving sharper boundaries than the standard Perona-Malik approach. All mathematical derivations were implemented from scratch using only Java's standard math library, without any external image processing frameworks, demonstrating applied skills in numerical methods, matrix operations, and algorithm implementation from first principles.",
    status: "Completed",
    duration: "1 month",
